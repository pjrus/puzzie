...
export default function PuzzlesPage() {
  ...
  useEffect(() => {
    ...
    if (filters.includes(query as CategoryFilter))
      setFilter(query as CategoryFilter);
  }, []);
  ...
  const grouped = useMemo(
    () =>
      typeOrder
        .map((type) => ({
          type,
          items: puzzles.filter(
            (puzzle) =>
              puzzle.type === type && matchesFilter(puzzle.type, filter),
          ),
        }))
  ...
  return (
    <div className="page-width pb-20 pt-10 lg:pt-16">
      ...
      >
        {filters.map((item) => (
          <button
            ...
            className={`min-h-11 shrink-0 rounded-full border px-5 text-sm font-black transition-colors ${filter === item ? "border-[var(--ink)] bg-[var(--ink)] text-[var(--surface)]" : "border-[var(--line)] bg-[var(--surface)] text-[var(--ink-muted)] hover:border-[var(--ink)] hover:text-[var(--ink)]"}`}
          >
            {item}
          </button>
        ))}
      ...
      <div key={filter} className="mt-10 space-y-14">
          ...
          return (
            <section key={group.type} aria-labelledby={`${group.type}-heading`}>
              ...
              <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                {group.items.map((puzzle, puzzleIndex) => (
                  <PuzzleCard
                    key={puzzle.id}
                    puzzle={puzzle}
                    compact
                    motionDelay={Math.min(
                      180,
                      groupIndex * 35 + puzzleIndex * 45,
                    )}
                  />
                ))}
              </div>
      ...
      {grouped.length === 0 && (
        <div className="surface-card mt-10 p-10 text-center">
          <Icon
            name="spark"
            size={28}
            className="mx-auto text-[var(--coral)]"
          />
          <h2 className="mt-4 text-2xl font-semibold">
            No puzzles in that corner yet.
          </h2>
          <p className="mt-2 text-[var(--ink-muted)]">
            Try another category and your next little challenge is waiting.
          </p>
        </div>
      )}
    </div>
