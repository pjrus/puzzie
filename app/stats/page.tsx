        ...
        <>
          <div className="mt-8 grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <section className="surface-card p-6 sm:p-8">
              ...
              <div className="mt-6 space-y-4">
                  ...
                  .map((type) => (
                    <div key={type}>
                      ...
                      <div className="h-2 overflow-hidden bg-[var(--line)]">
                        <div
                          className="progress-fill h-full bg-[var(--coral)]"
                          style={{
                ...
                {statTypes.every(
                  (type) => progress.categoryStats[type] === 0,
                ) && (
                  <p className="text-[var(--ink-muted)]">
                    Your category chart will appear after your first puzzle.
                  </p>
                )}
              </div>
            </section>
          ...
          <section className="mt-8">
            <SectionHeading
              ...
              description="Your last few puzzles, newest first."
            />
            <div className="surface-card mt-6 overflow-hidden">
              {history.slice(0, 8).map((entry, index) => (
                <div
                  key={`${entry.id}-${entry.completedAt}`}
                  className={`flex flex-wrap items-center justify-between gap-4 p-4 sm:p-5 ${index > 0 ? "border-t border-[var(--line)]" : ""}`}
                >
              ...
              {history.length === 0 && (
                <p className="p-6 text-[var(--ink-muted)]">No results yet.</p>
              )}
            </div>
          </section>
        </>
